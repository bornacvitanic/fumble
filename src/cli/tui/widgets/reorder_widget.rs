...
impl ReorderWidget<'_> {
    ...
    pub fn set_delay_duration(&mut self, delay_duration_ms: u64) {
        self.delay_duration_text_area
            .set_text(&delay_duration_ms.to_string());
            ...
            match self.selected {
                ...
                1 => {
                    if self.delay_duration_text_area.input(key) {
                        self.delay_duration =
                            u64::parse_from_text_area(&self.delay_duration_text_area);
                    }
                }
                _ => {}
            }
...
impl Widget for &mut ReorderWidget<'_> {
        ...
        Self: Sized,
    {
        ...
        if !self.delay_duration_text_area.lines()[0].is_empty() {
            style_textarea_based_on_validation(
                &mut self.delay_duration_text_area,
                &self.delay_duration,
            );
        }
        self.delay_duration_text_area
            .render(delay_duration_area, buf);
        ...
        .areas(info_area);
        Paragraph::new(format!("{:.2}%", self.reorder_rate * 100.0))
            .block(Block::bordered().title("Reorder rate"))
            .render(reorder_percentage_info, buf);
        Paragraph::new(format!("{}", self.delayed_packets))
            .block(Block::bordered().title("Delayed"))
            .render(delayed_count_info, buf);
    }
}